...
@router.post("/", response_model=RemitoVentaResponse, status_code=status.HTTP_201_CREATED)
async def create_remito_venta_endpoint(
    remito_data: RemitoVentaCreateRequest,
...
async def get_remito_venta_by_id_endpoint(
    ...
    if db_remito is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Remito de venta no encontrado")
    return db_remito
...
async def update_remito_venta_endpoint(
    ...
    updated_remito = await remito_venta_dal.update_remito_venta(db, remito_id, remito_update)
    if updated_remito is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Remito de venta no encontrado para actualizar")
...
async def delete_remito_venta_endpoint(
    ...
    if not was_deleted:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Remito de venta no encontrado para eliminar")
    return 