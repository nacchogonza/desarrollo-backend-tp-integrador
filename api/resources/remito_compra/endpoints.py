...
router = APIRouter(
    prefix="/remitos-compra",
    tags=["Remitos Compra"],
    responses={404: {"description": "Remito de Compra no encontrado"}},
)

@router.post("/", response_model=RemitoCompraResponse, status_code=status.HTTP_201_CREATED)
async def create_remito_compra_endpoint(
    remito_data: RemitoCompraCreateRequest,
...
async def get_remito_compra_by_id_endpoint(
    ...
    if db_remito is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Remito de compra no encontrado")
    return db_remito
...
async def update_remito_compra_endpoint(
    ...
    updated_remito = await remito_compra_dal.update_remito_compra(db, remito_id, remito_update)
    if updated_remito is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Remito de compra no encontrado para actualizar")
...
async def delete_remito_compra_endpoint(
    ...
    if not was_deleted:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Remito de compra no encontrado para eliminar")
